...
#+begin_src scheme :noweb-ref service
  (service dhcpcd-service-type
  	 (dhcpcd-configuration
  	  ...
  	  (static '("domain_name_servers=100.100.100.100 192.168.122.1" "domain_search=tailb8a678.ts.net"))))
...
#+begin_src scheme :noweb-ref mcron-job
  (job "0 2 * * *" "doas -u misskey pg_dump -U misskey -d misskey > /tmp/misskey.sql && ( rsync -avz --timeout=30 /tmp/misskey.sql rsync://100.119.107.8/backup ; rm /tmp/misskey.sql)")
#+end_src
    ...
    (computed-file "sing-box.json"
      (with-extensions (map specification->package '("guile-json@4"))
        #~(begin
            ...
            (output-singbox-streamer #$output)))))
#+end_src
