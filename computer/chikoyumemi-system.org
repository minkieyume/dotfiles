...
#+begin_src scheme :noweb-ref user
  (user-account
   ...
   (supplementary-groups '("wheel" "netdev" "audio" "video" "yumemi" "docker")))
#+end_src
...
(service kernel-module-loader-service-type
	 '("uvcvideo" "videodev" "v4l2loopback"))
#+end_src
...
** 应用扩展
#+begin_src scheme :noweb-ref application
os-sys-app
#+end_src
...
** Home服务
#+begin_src scheme :noweb-ref define
(load "../home/minkieyume-home-pc.scm")
#+end_src

#+begin_src scheme :noweb yes :noweb-prefix no :noweb-ref service
(service guix-home-service-type
    	 `(("minkieyume" ,((compose <<home-transformation>>)
  			   minkieyume-home-pc))))
#+end_src

添加服务：
#+begin_src scheme :noweb yes :noweb-prefix no :noweb-ref home-transformation
(lambda (home)
  (home-environment
    (inherit home)
    (services (append (list <<home-service>>) (home-environment-services home)))))
#+end_src
...
(simple-service 'chiko-home-files
		...
		`((".dash_rsa" ,(local-file "../secret/keys/dash_rsa"))
		  ...
		  (".ssh/environment" ,(local-file "../files/config/ssh-env"))
...
#+begin_src scheme :noweb-ref home-service
(simple-service 'emacs-init-themes
...
(job '(next-minute (range 0 60 3))
     "mbsync -a > /tmp/mbsync.log 2>&1")
#+end_src

#+begin_src scheme :noweb-ref home-service
(simple-service 'isync-service
  		home-files-service-type 
  		`((".mbsyncrc"
  		   ,(local-file "../files/config/mbsyncrc.conf"))))
#+end_src
...
*** MIME
#+begin_src scheme :noweb-ref home-service :noweb yes :noweb-prefix no
...
#+begin_src scheme :noweb-ref mime
...
#+begin_src scheme :noweb-ref home-service
(simple-service 'cc-wrapper
		home-files-service-type
		`((".local/bin/cc" ,(symlink-to (file-append (spec->pkg "gcc-toolchain") "/bin/gcc")))))
#+end_src
...
** Qdrant
#+begin_src scheme :noweb-ref service
(service qdrant-service-type
	 (qdrant-configuration
	  (image "qdrant/qdrant:v1.15.5-gpu-nvidia")
	  (api-key (nyapasu-ref 'qdrant-api-key))))
#+end_src

** Aider
Aider暂时没有包，需要用uvx安装。
#+begin_src scheme :noweb-ref home-service
(simple-service 'aider-file
		home-files-service-type
		`((".local/bin/aider" ,aider-wrapper)))
#+end_src
