...
(define cc-wrapper
  (program-file
   "cc-wrapper"
   #~(begin
       (apply execl (cons #$(file-append (spec->pkg "gcc-toolchain") "/bin/gcc") (command-line))))))
#+end_src

#+begin_src scheme
(simple-service 'cc-wrapper
		home-files-service-type
		`((".local/bin/cc" ,cc-wrapper)))
#+end_src
...
然后使用patchelf命令来修复路径问题：
#+begin_src shell
patchelf --set-interpreter /gnu/store/py9r4p4jp2pa1jaagbcvw35d79j5kcg5-glibc-2.41/lib/ld-linux-x86-64.so.2 ./uv
patchelf --set-interpreter /gnu/store/py9r4p4jp2pa1jaagbcvw35d79j5kcg5-glibc-2.41/lib/ld-linux-x86-64.so.2 ./uvx
#+end_src
