...
(let ((proxy-confs (list "proxy_http_version 1.1;"
  ...
  (service nginx-service-type
	     ...
	     (stream (nginx-stream-configuration
		      (upstream-blocks
		       (list <<nginx-stream-upstream>>))
		      (server-blocks
		       (list <<nginx-stream-server>>)))))))
#+end_src
...
#+begin_src scheme :noweb-ref nginx-server
(nginx-server-configuration
  (server-name '("vcs.yumieko.com"))
  (listen '("443 ssl" "[::]:443 ssl"))
  (ssl-certificate "/etc/certs/vcs.yumieko.com/fullchain.pem")
  (ssl-certificate-key "/etc/certs/vcs.yumieko.com/privkey.pem")
  (locations
   (list
    (nginx-location-configuration
      (uri "/")
      (body `("proxy_pass http://100.119.107.8:3020;"
	      ,@proxy-confs))))))
#+end_src

**** SSH代理
#+begin_src scheme :noweb-ref nginx-stream-upstream
(nginx-upstream-configuration
 (name "forgejo_ssh")
 (servers (list "100.119.107.8:2222")))
#+end_src

#+begin_src scheme :noweb-ref nginx-stream-server
(nginx-server-configuration
  (server-name '("vcs.yumieko.com"))
  (listen '("2222" "[::]:2222"))
  (locations
