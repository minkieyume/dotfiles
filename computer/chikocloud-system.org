...
(service certbot-service-type
	     ...
	     (certificate-configuration
	      (domains '("dimension.yumieko.com")))
	     (certificate-configuration
	      (domains '("etherpad.yumieko.com")))))))
#+end_src
...
#+begin_src scheme :noweb-ref nginx-server
(nginx-server-configuration
  (server-name '("dimension.yumieko.com"))
  (listen '("443 ssl" "[::]:443 ssl"))
  (ssl-certificate "/etc/certs/dimension.yumieko.com/fullchain.pem")
  (ssl-certificate-key "/etc/certs/dimension.yumieko.com/privkey.pem")
  (locations
   (list
    (nginx-location-configuration
      (uri "/")
      (body `("proxy_pass http://127.0.0.1:8184;"
	      ,@proxy-confs))))))
#+end_src
...
#+begin_src scheme :noweb-ref nginx-server
(nginx-server-configuration
  (server-name '("etherpad.yumieko.com"))
  (listen '("443 ssl" "[::]:443 ssl"))
  (ssl-certificate "/etc/certs/etherpad.yumieko.com/fullchain.pem")
  (ssl-certificate-key "/etc/certs/etherpad.yumieko.com/privkey.pem")
  (locations
   (list
    (nginx-location-configuration
      (uri "/")
      (body `("proxy_pass http://127.0.0.1:9001;"
	      ,@proxy-confs))))))
#+end_src
...
(service etherpad-service-type
	 (etherpad-configuration
	  (etherpad "minkieyume/etherpad:latest")
	  (uid 5001)
	  (gid 5001)
...
重构时插件参数配置，该参数用于本地重构docker仓库时快速添加插件。
#+begin_src conf
ETHERPAD_PLUGINS="ep_comments_page ep_author_neat ep_markdown ep_padlist2 ep_headings2 ep_auth_session"
#+end_src
