...
(simple-service 'postgres-pass-file
		...
		`(("secret/postgres/hedgedoc" ,(plain-file "hedgedoc-db-password.txt"
		  ...
		  ("secret/postgres/etherpad" ,(plain-file "etherpad-db-password.txt"
							     (nyapasu-ref 'eltherpaddb)))
		  ("secret/postgres/authentik" ,(plain-file "authentik-db-password.txt"
							       (nyapasu-ref 'authentik-db-pass)))))
#+end_src
...
#+begin_src scheme :noweb-ref nginx-server
(nginx-server-configuration
  (server-name '("auth.yumieko.com"))
  (listen '("443 ssl" "[::]:443 ssl"))
  (ssl-certificate "/etc/certs/auth.yumieko.com/fullchain.pem")
  (ssl-certificate-key "/etc/certs/auth.yumieko.com/privkey.pem")
  (locations
   (list
    (nginx-location-configuration
      (uri "/")
    ...
    (nginx-location-configuration
      (uri "/misskey-auth")
      (body `("alias /etc/www/authentik/misskey-auth;"
	      "index client.html;"
	      "try_files $uri $uri/ =404;"))))))
#+end_src

#+begin_src scheme :noweb-ref service
(simple-service 'authentik-misskeyauth
		etc-service-type
		`(("www/authentik/misskey-auth/client.html" ,(local-file "../files/www/authentik/oauth-client.html"))))
#+end_src
...
** Authentic
#+begin_src scheme :noweb-ref service
(service authentik-service-type
         (authentik-configuration
          (secret-key (nyapasu-ref 'authentik-secret-key))
          (postgresql-password-file "/etc/secret/postgres/authentik")
          (db-password (nyapasu-ref 'authentik-db-pass))
          (http-port 9000)
          (https-port 9443)))
#+end_src
